...
class KoladaDar
{
    ...
    public static function init($fields)
    {
        ...
        if (isset($fields['DateGrigorFirst'])) {
            if (is_null($fields['DateGrigorFirst'])) {
                $d = date('d');
                $m = date('m');
                if ($m < 9 and $d < 22) {
                    $y = date('Y') - 1;
    ...
    private function head()
    {
        $headers = [
            ['name' => '#'],
            ['name' => 'Название'],
            ['name' => '1', 'options' => $this->getValue($this->optionsColumn, 1, [])],
            ['name' => '2', 'options' => $this->getValue($this->optionsColumn, 2, [])],
            ['name' => '3', 'options' => $this->getValue($this->optionsColumn, 3, [])],
            ['name' => '4', 'options' => $this->getValue($this->optionsColumn, 4, [])],
            ['name' => '5', 'options' => $this->getValue($this->optionsColumn, 5, [])],
            ['name' => '6', 'options' => $this->getValue($this->optionsColumn, 6, [])],
            ['name' => '1', 'options' => $this->getValue($this->optionsColumn, 1, [])],
            ['name' => '2', 'options' => $this->getValue($this->optionsColumn, 2, [])],
            ['name' => '3', 'options' => $this->getValue($this->optionsColumn, 3, [])],
            ['name' => '4', 'options' => $this->getValue($this->optionsColumn, 4, [])],
     ...
     * @return mixed|null
     */
    private function getValue($array, $index, $default = null)
    {
    ...
    private static function tag($name, $content, $options = [])
    {
        ...
        foreach ($options as $k => $v) {
            $ve = htmlspecialchars($v, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8', true);
            $rows[] = $k . '=' . '"' . $ve . '"';
        }
        if (count($rows) > 0) $o = ' ' . join(' ', $rows);
        else $o = '';

        return '<' . $name . $o . '>' . $content . '</' . $name .'>' ;
    }
